  ...
  const handleJoinGame = async () => {
    if (!gameCode.trim()) {
      toast({
        ...
        description: 'Por favor, insira um código de partida.',
      });
      return;
    }
    ...
    if (!firestore || !user) {
      toast({
        ...
        description: 'Você precisa estar logado para entrar em um jogo.',
      });
      return;
    }
    ...
    try {
      ...
      if (!players[user.uid]) {
        const newPlayer = {
          name: user.displayName || `Anônimo ${Object.keys(players).length + 1}`,
          role: 'influencer', // Default role, can be changed in lobby
